...
import { assertUserOwnsWorkspace, assertUserOwnsList, assertUserOwnsCard } from '@/modules/workspace/actions/assertions';
...
async function ensureDefaultWorkspace(userId: string, workspaces: any[]) {
  ...
  const { data: cw, error } = await supabase.from('workspaces').insert({ user_id: userId, name: 'My Workspace' }).select().single();
  if (error) throw new Error(`Failed to generate default workspace: ${error.message}`);
  
  await supabase.from('workspace_members').insert({ workspace_id: cw.id, user_id: userId, role: 'owner' });
  return [cw];
}
...
async function extractNewMentions(oldDesc: string | null | undefined, newDesc: string | null | undefined) {
  const oldMentions = oldDesc ? Array.from(oldDesc.matchAll(/(?:^|\s)@(\w+)/g)).map(m => m[1]) : [];
  const newMentions = newDesc ? Array.from(newDesc.matchAll(/(?:^|\s)@(\w+)/g)).map(m => m[1]) : [];
  return newMentions.filter(m => !oldMentions.includes(m));
}

async function notifyMentionedUsers(addedMentions: string[], userId: string, cardId: string) {
  if (addedMentions.length === 0) return;
  
  const { data: users } = await supabase.from('users').select('id, username').in('username', addedMentions);
  if (!users || users.length === 0) return;
  
  const notifications = users.map(u => ({
    user_id: u.id, actor_id: userId, card_id: cardId, type: 'mention_desc', content: 'You were mentioned in a card description.'
  }));
...
export async function updateCardDetailsAction(id: string, updates: Partial<Card>) {
  ...
  if (oldCard) {
     ...
     if (updates.description !== undefined && oldCard.description !== updates.description) {
        ...
        const addedMentions = await extractNewMentions(oldCard.description, updates.description);
...
export async function updateCardPositionsAction(updates: { id: string, list_id: string, position: number }[]) {
  ...
  await Promise.all(uniqueListIds.map(listId => assertUserOwnsList(userId, listId)));
