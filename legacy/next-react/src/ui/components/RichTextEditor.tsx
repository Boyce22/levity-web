...
function markdownToSlice(text: string, schema: Schema) {
...
export function RichTextEditor({
  ...
  const handleDrop = (view: EditorView, event: DragEvent, _slice: unknown, moved: boolean) => {
  ...
  const handlePaste = (view: any, event: ClipboardEvent) => {
    ...
    if (fileItem) {
      ...
      uploadFile(file, workspaceIdRef.current)
        ...
        .catch((e: any) => alert('Paste failed: ' + (e instanceof Error ? e.message : String(e))))
        .finally(() => setIsUploading(false));
    ...
    >
      ...
      >
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-[12px] font-medium transition-all"
          style={{
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-[12px] font-medium text-[var(--app-text-muted)] transition-all hover:bg-[var(--app-hover)] hover:text-[var(--app-text)] disabled:opacity-40"
